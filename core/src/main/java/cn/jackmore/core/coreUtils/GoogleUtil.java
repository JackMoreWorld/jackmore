...
public class GoogleUtil {
  ...
  /*  public static boolean  check_code(long code, long timeMsec,Integer userId) { //secret写死。code前台传。timeMsec系统时间 精确到s
        Base32 codec = new Base32();
        ICommonService commonService=(ICommonService) SpringUtil.getBean("commonServiceImpl");
        String str = commonService.querySystemConfigInfo(1).getGoogleSecret();
        byte[] decodedKey = codec.decode(str);
    ...
    public static boolean checkGoogleCode(long googleCode,long timeMsec,String googleSecret){
        Base32 codec = new Base32();
        ...
        for (int i = -window_size; i <= window_size; ++i) {
            ...
            try {
                hash = verify_code(decodedKey, t + i);
                System.out.println("hash="+hash+"--googleCode="+googleCode+"--timeMsec"+timeMsec);
            }catch (Exception e) {
                e.printStackTrace();
                throw new RuntimeException(e.getMessage());
            }
